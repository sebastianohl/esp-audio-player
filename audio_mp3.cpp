...
struct read_data_instance;
typedef struct read_data_instance read_data_instance;

struct read_data_instance
{
    size_t (*read_func)(read_data_instance *params, uint8_t *ptr, size_t size);
    bool eof;
    union
    {
        struct
...
size_t read_file(read_data_instance *params, uint8_t *ptr, size_t size)
{
    size_t nRead = fread(ptr, 1, size, params->fp);
    if ((nRead == 0) || feof(params->fp))
    {
        params->eof = true;
    }
    return nRead;
...
size_t read_stream(read_data_instance *params, uint8_t *ptr, size_t size)
{
    return esp_http_client_read(params->client, (char *)ptr, size);
}

/**
 * @return true if data remains, false on error or end of file
 */
DECODE_STATUS decode_mp3_internal(HMP3Decoder mp3_decoder, read_data_instance *read_data, decode_data *pData, mp3_instance *pInstance)
{
    ...
    if (unread_bytes < 1.25 * MAINBUF_SIZE && !pInstance->eof_reached)
    {
        ...
        size_t nRead = read_data->read_func(read_data, write_ptr, free_space);
        pInstance->bytes_in_data_buf = unread_bytes + nRead;
        pInstance->read_ptr = pInstance->data_buf;

        if ((nRead == 0) || read_data->eof)
        {
...
{
    read_data_instance data = {
        .read_func = &read_file,
        .eof = false,
        .fp = fp};
    return decode_mp3_internal(mp3_decoder, &data, pData, pInstance);
}
